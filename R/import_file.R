...
#' data <- import_file(path, cols = c("col1", "col2"),
...
import_file <- function(path, cols = NULL, cols_new = NULL, exprs = NULL,
  ...
  if (!file.exists(path)) {
    ...
    if (!is.null(cols)) {
      loaded_file <- fread(path, stringsAsFactors = F, select = cols,
                    na.strings = c("", NA), colClasses = c("character"))
    } else {
      loaded_file <- fread(path, stringsAsFactors = F, na.strings = c("", NA),
    ...
    invisible(lapply(colnames(loaded_file), function(x) loaded_file <- loaded_file[, eval(x) :=
                                                                trimws(get(x), "b", whitespace = "[\\h\\v]")]))
    loaded_file[loaded_file == ""] <- NA
    
    # Set specified columns in data.colls to date format
    if (!is.null(date_cols)) {
      lapply(date_cols, function(x) loaded_file[, eval(x) :=
                                           as.Date(get(x), "%Y%m%d")])
    }
