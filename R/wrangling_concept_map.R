...
#' melting multiple `column_name_*` and `expected_value_*` columns,
#' and assigns an `id_set` identifier to each original mapping entry.
#'
...
#'        Column name containing the date associated with the record.
#'      }
#'     \item{column_name_*}{One or more columns specifying CDM columns to map.}
#'     \item{expected_value_*}{One or more columns specifying the expected code
#'        values corresponding to the CDM columns.}
#'   }
#'
#' @details
#' This function:
...
#'   \item Dynamically identifies all `column_name_*`
#'      and `expected_value_*` columns.
#'   \item Converts the mapping into a long format,
...
#' long_map <- wrangling_concept_map(dap_specific_concept_map)
#' }
#'
#' @import data.table
#' @export
#'
wrangling_concept_map <- function(dap_specific_concept_map) {
  # ---- Input checks ----
  if (!data.table::is.data.table(dap_specific_concept_map)) {
    stop("[wrangling_concept_map] Input must be a data.table.")
  }

  if (nrow(dap_specific_concept_map) == 0) {
    stop("[wrangling_concept_map] Input data.table is empty.")
  }
  ...
  if (length(missing_cols) > 0) {
    stop(
      "[wrangling_concept_map] Missing required columns: ",
  ...
  if (length(column_names) == 0) {
    stop("[wrangling_concept_map] No 'column_name_*' columns found.")
  }
  if (length(expected_values) == 0) {
    stop(
      "[wrangling_concept_map] No 'expected_value_*' columns found."
    )
  }
  # Add an ID column
  ...
  column_names <- names(dap_specific_concept_map)[
    grep("^column_name_", names(dap_specific_concept_map))
  ]
  expected_values <- names(dap_specific_concept_map)[
    grep("^expected_value_", names(dap_specific_concept_map))
  ]
