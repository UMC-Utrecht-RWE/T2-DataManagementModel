...
#'
#' This function retrieves unique code lists from a database based on the specified columns.
#'
#' @param db_connection A database connection object.
#' @param column_specs A list of lists specifying the columns to be used for distinct, along with their new names.
#'        Each inner list should contain two elements: "column_name" and "new_name".
#'
...
#' result_list <- getUniqueCodeList(db_connection_origin, column_info_list)

#' # Close the database connection
#' dbDisconnect(db_connection_origin)

#' # Access individual results from the list
#' UNIQUE_CODELIST_eventCode <- result_list[[1]]
#' UNIQUE_CODELIST_freeText <- result_list[[2]]
#' }
#'
#' @export
#'

getUniqueCodeList <- function(db_connection, column_info_list) {
  ...
  for (query in queries) {
    result <- as.data.table(dbGetQuery(db_connection, query))
