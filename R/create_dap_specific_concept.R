...
create_dap_specific_concept <- function(codelist, data_db, name_attachment, save_db, date_col_filter = NULL,
  ...
  if (length(cols_names) == 0 | length(value_names) == 0| length(cols_names) != length(value_names)) {
    ...
    print(paste0("[create_dap_specific_concept] Identified column names: ", paste0(cols_names, collapse = ",")))
    print(paste0("[create_dap_specific_concept] Identified value names: ", paste0(value_names, collapse = ",")))
