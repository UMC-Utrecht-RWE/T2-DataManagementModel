...
#'
#' This function sets up the `concept_table` within the database.
#' Depending on the \code{type_table} argument, it either creates a persistent
#' SQL table with constraints or a virtual view that reads from Parquet
#' files using DuckDB's \code{read_parquet} functionality.
#'
#' @param con A database connection object (typically a DuckDB connection).
#' @param type_table Character. Either \code{"view"} or \code{"table"}.
#' If \code{"view"}, the function creates a view pointing to Parquet files.
#' If \code{"table"}, it creates an empty physical table with unique constraint.
...
#' @param overwrite Logical. If \code{TRUE}, an existing \code{concept_table}
#'  will be dropped before being re-initialized. Defaults to \code{FALSE}.
#' @param add_id_set Logical. If \code{TRUE} (default),
#' includes the \code{id_set} column in the table schema or view definition.
#'
#' @details
...
initialize_concept_table <- function(con,
  ...
  if (DBI::dbExistsTable(con, "concept_table") &&  overwrite == TRUE) {
    ...
    DBI::dbRemoveTable(con, "concept_table")
  }
