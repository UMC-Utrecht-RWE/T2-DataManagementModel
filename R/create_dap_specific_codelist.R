...
create_dap_specific_codelist <- function(
  ...
  if (!is.character(unique_codelist$coding_system) && !is.factor(unique_codelist$coding_system)) {
    stop("unique_codelist$coding_system must be character or factor")
  }
  
  if (!is.character(study_codelist$coding_system) && !is.factor(study_codelist$coding_system)) {
  ...
  if (!is.na(priority)) {
    ...
    if (!priority %in% names(unique_codelist)) {
      stop(paste("priority column '", priority, "' not found in unique_codelist"))
    }
  }
