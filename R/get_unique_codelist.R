...
#'
#' @return A list of data.tables with distinct values based on the specified columns.
#' @export
get_unique_codelist <- function(db_connection, column_info_list, tb_name) {
  ...
  for (i in seq_along(column_info_list)) {
    ...
    if (!all(required_names %in% names(item))) {
      stop(sprintf("Element [[%d]] in column_info_list is missing 'source_column' 
                   or 'alias_name'.", i))
    }
    
    # Ensure source and alias have the same length for mapping
    if (length(item$source_column) != length(item$alias_name)) {
      stop(sprintf("In element [[%d]], 'source_column' and 'alias_name' 
  ...
  for (col_info in column_info_list) {
    ...
    group_by_clause <- sprintf("GROUP BY %s", paste(source_column, collapse = ", "))
