...
load_db <- function(
  ...
  for (table in cdm_tables_names) {
    ...
    if (length(available_date_cols) == 0) {
      ...
      cols_msg <- paste(available_date_cols, collapse = ", ")
      message(paste0(
        "[load_db]: The following columns are identified with date format: ",
        cols_msg
      ))

      id <- function(x) DBI::dbQuoteIdentifier(db_connection, x)

      alter_to_date <- function(col) {
        DBI::dbExecute(
          db_connection,
          sprintf(
            "ALTER TABLE %s ALTER %s TYPE DATE;",
            id(table), id(col)
          )
        )
      }

      fix_with_strptime <- function(col) {
        message(paste0(
          "      Direct conversion failed for ",
          col,
          ". Attempting to fix with STRPTIME."
        ))

        if (identical(selected_format, "parquet")) {
          tryCatch(
            {
              DBI::dbExecute(
                db_connection,
                sprintf(
                  "UPDATE %s
                  SET %s = CAST(%s AS DATE)
                  WHERE %s IS NOT NULL;",
                  id(table), id(col), id(col), id(col)
                )
              )
            },
            error = function(e2) {
              # Null out values that don't match yyyyMMdd or yyyy-MM-dd
              DBI::dbExecute(
                db_connection,
                sprintf(
                  "UPDATE %s
                  ...
                  WHERE %s IS NOT NULL
                    AND NOT (%s ~ '^[0-9]{8}$'
                              OR %s ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$');",
                  id(table), id(col), id(col), id(col), id(col)
                )
              )
    ...
    invisible(lapply(available_character_cols, function(new_column) {
      message(paste0(
        "  Cleaning character column from empty spaces: ",
        new_column
      ))
      tryCatch(
        {
