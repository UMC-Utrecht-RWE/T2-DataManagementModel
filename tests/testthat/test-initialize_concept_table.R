testthat::test_that("initialize_concept_table handles overwriting and schema", {
...
testthat::test_that("initialize_concept_table handles view creation and invalid inputs", {
  con <- dbConnect(duckdb::duckdb(), ":memory:")
  on.exit(dbDisconnect(con, shutdown = TRUE))
  ...
  res <- dbGetQuery(
    ...
    WHERE table_name = 'concept_table'"
  )
