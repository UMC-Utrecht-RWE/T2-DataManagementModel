test_that("Checking the result is a data.table", {
  ...
  unique_codelist <- get_unique_codelist(db_connection = db_con, 
                                         column_info_list = column_info_list, 
                                         tb_name = 'MEDICINES')[[1]]
  
  # The MEDICINES table is a mix of long-formatted and wide-formatted information, 
  # the codes are hosted in a wide-formatted field where they miss the coding system
...
test_that("Checking the check messages of the expected format of the codelist 
          and unique codelist work", {
            ...
            # Extract unique codelist and ensure it's a data.table
            unique_codelist <- as.data.table(get_unique_codelist(db_connection = db_con, 
                                                                 column_info_list = column_info_list, 
                                                                 tb_name = 'MEDICINES')[[1]])
            ...
            # Test 2: Verify error when unique_codelist is missing required 'coding_system' column
            testthat::expect_error(dap_specific_codeslist <- T2.DMM::create_dap_specific_codelist(
              unique_codelist = unique_codelist, 
              study_codelist = study_codelist),
            ...
            # Add coding_system column with wrong type (numeric instead of character/factor)
            unique_codelist[, coding_system := 1] 
            # Standardize study_codelist column names
            data.table::setnames(x = study_codelist, old = 'drug_abbreviation', 'concept_id')
            data.table::setnames(x = study_codelist, old = 'product_identifier', 'coding_system')
            
            # Test 6: Verify error when unique_codelist$coding_system has wrong data type
            testthat::expect_error(dap_specific_codeslist <- T2.DMM::create_dap_specific_codelist(
              unique_codelist = unique_codelist,
              study_codelist = study_codelist),
            ...
            unique_codelist[, coding_system := NULL][, coding_system := 'PRODCODEID']
            # Break study_codelist by removing coding_system column
            data.table::setnames(x = study_codelist, old = 'coding_system', new = 'product_identifier')
            
            # Test 7: Verify error when study_codelist is missing required 'coding_system' column
            testthat::expect_error(dap_specific_codeslist <- T2.DMM::create_dap_specific_codelist(
              unique_codelist = unique_codelist,
              study_codelist = study_codelist),
            ...
            # Test 8: Verify error when study_codelist$coding_system has wrong data type
            # Add coding_system column with wrong type (numeric instead of character/factor)
