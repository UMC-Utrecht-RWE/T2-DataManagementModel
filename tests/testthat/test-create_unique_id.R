...
## VIEW TESTS
test_that("VIEW: Names and number of new columns added", {
  db_con <- create_loaded_test_db()
  withr::defer(DBI::dbDisconnect(db_con))
  ...
  persons_db <- DBI::dbReadTable(db_con, "view_PERSONS")
  expect_contains(names(persons_db), c("ori_id", "ROWID", "ori_table"))
})

test_that("VIEW: Checking the ROWID is the same as the number of rows of the table", {
  db_con <- create_loaded_test_db()
  withr::defer(DBI::dbDisconnect(db_con))
...
test_that("VIEW: Checking the OriTable is the same as the included table", {
  db_con <- create_loaded_test_db()
  withr::defer(DBI::dbDisconnect(db_con))
...
test_that("VIEW: With order_by_cols", {
  db_con <- create_loaded_test_db()
  withr::defer(DBI::dbDisconnect(db_con))
